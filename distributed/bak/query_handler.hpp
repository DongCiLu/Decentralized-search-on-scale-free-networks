...
class query_handler{
    public:
...
#endif
                size_t err_cnt = 0) :
            n_tree(n_tree), stepy(stepy), n_query(n_query), 
        ...
        void ds_query_batch() {
            dc->cout() << "\n1 Start building index..." << std::endl;
            for (size_t tree_id = 0;
            build_index(n_tree);
        ...
        void perform_ds() {
            ...
            for (size_t i = 0; i < n_query; i += n_query_batch) {
                ...
                dc->cout() << "\t2.4 Gather results..." << std::endl;
            ...
            if (procid == masterid) {
...
#else
                    comb_res.insert(results[i].begin(), 
                            results[i].end());
#endif
                }
                ...
                for (std::map<size_t, gsInstance>::iterator 
                    ...
                    if (iter->second.real_dist == 0) {
                        ...
                        sum_appr_err += 
                            double(iter->second.path.size() - 1) /
