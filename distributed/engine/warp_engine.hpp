 ...
 **************************************************************************/
  ...
  public:
    ...
    void signal_pairs(std::vector<vertex_id_type> &gvid_set,
            std::vector<message_type> &msg_set) {
      logstream(LOG_DEBUG) << rmi.procid() << ": Schedule All (Specialized)" << std::endl;
      for(size_t i = 0; i < gvid_set.size(); ++i) {
        if(graph.is_master(gvid_set[i])) {
            lvid_type lvid = graph.vertex(gvid_set[i]).local_id();
            double priority;
            messages.add(lvid, msg_set[i], &priority);
            scheduler_ptr->schedule(lvid, priority);
        }
      }
      rmi.barrier();
